import { Badge, Card, Container, Group, Modal, Skeleton, Stack, Text, Title } from '@mantine/core';
import { useQuery } from '@tanstack/react-query';
import { useNavigate } from '@tanstack/react-router';
import { useAtom } from 'jotai';
import { useEffect, useState } from 'react';

import { QuestionMentorItem, getActiveApps } from '../api';
...
export const ActiveTasksPage = () => {
  const { isLoading, data } = useQuery({
    queryKey: ['applications-active'],
    queryFn: getActiveApps
  });
  ...
  return (
    <Container size='xl' p='md' style={{ minHeight: '100vh', width: '100%' }}>
      <Stack>
        ...
        <Title order={2} mb='md'>
          Всего взятых заявок: {applications?.length}
        </Title>
      </Stack>
